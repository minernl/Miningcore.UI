// config
var API = 'http://localhost/api/'; // Change this in your public API address
var defaultPool = 'YOUR_POOL_ID'; // Default Pool ID
...
function loadStatsData() {
    return $.ajax(API + 'pools')
        .done(function (data) {
            $.each(data.pools, function (index, value) {
                if (currentPool === value.id) {
                    $('#paymentMinimumPayment').text(_formatter(value.paymentProcessing.minimumPayment, 0, ' XMR'));
					$('#paymentPayoutScheme').text(value.paymentProcessing.payoutScheme);
					//$('#poolShares').text(_formatter(value, 0, ''));
                    //$('#poolBlocks').text(_formatter(value, 0, ''));
					$('#poolTotalPaid').text(_formatter(value.totalPaid, 0, ' XMR'));
					...
					$('#poolFeePercent').text(_formatter(value.poolFeePercent, 1, '%'));
					
					$('#networkBlockHeight').text(value.networkStats.blockHeight);
					...
					$('#networkLastBlock').text((convertUTCDateToLocalDate(new Date(value.networkStats.lastNetworkBlockTime))).toLocaleString());
                }
            });
        })
        .fail(function () {
        ...
        .done(function (data) {
            ...
            if (data.length > 0) {
                $.each(data, function (index, value) {
                    ...
                    minerList += '<td>' + _formatter(value.sharesPerSecond, 5, 'Shares/s') + '</td>';
                    minerList += '</tr>';
                });
            } else {
...
function loadBlocksList() {
    return $.ajax(API + 'pools/' + currentPool + '/blocks?pageSize=100')
        .done(function (data) {
            var blockList = '<thead><tr><th rowspan="2">Date &amp; Time</th><th>Height</th><th>Effort</th><th>Status</th><th>Reward</th><th style="display: none"></th><th colspan="4">Confirmation</th></tr></thead><tbody>';
            if (data.length > 0) {
                $.each(data, function (index, value) {
                    blockList += '<tr>';
...
function loadPaymentsList() {
    return $.ajax(API + 'pools/' + currentPool + '/payments?pageSize=500')
        .done(function (data) {
            var paymentList = '<thead><tr><th rowspan="2">Date &amp; Time</th><th>Address</th><th>Amount</th><th style="display: none"></th><th colspan="2">Confirmation</th></tr></thead><tbody>';
            if (data.length > 0) {
                $.each(data, function (index, value) {
                    paymentList += '<tr>';
                    ...
                    paymentList += '<td rowspan="2"><a href="' + value.transactionInfoLink + '" target="_blank">' + value.transactionConfirmationData.substring(0, 16) + ' &hellip; ' + value.transactionConfirmationData.substring(value.transactionConfirmationData.length - 16) + ' </a></td>';
                    paymentList += '</tr>';
                });
            } else {
            ...
            $.each(data.pools, function (index, value) {
                if (currentPool === value.id) {
                    ...
                    if (typeof(value.paymentProcessing.minimumPaymentToPaymentId) !== "undefined") {
                        connectPoolConfig += '<tr><td>Minimum Payout via Exchange</td><td>' + value.paymentProcessing.minimumPaymentToPaymentId + '</td></tr>';
                    }
...
function loadPoolPortList() {
    return $.ajax(API + 'pools')
        .done(function (data) {
            var PoolPortList = '<thead><tr><th>Item</th> <th>Value</th></tr></thead><tbody>';
			
            $.each(data.pools, function (index, value) {
                if (currentPool === value.id) {
					
                    //PoolPortList += '<tr><td>Algorithm</td><td>' + value.coin.algorithm + '</td></tr>';
                    //PoolPortList += '<tr><td>Wallet Address</td><td><a href="' + value.addressInfoLink + '" target="_blank">' + value.address.substring(0, 12) + ' &hellip; ' + value.address.substring(value.address.length - 12) + '</a></td></tr>';
                    //PoolPortList += '<tr><td>Payout Scheme</td><td>' + value.paymentProcessing.payoutScheme + '</td></tr>';
                    //PoolPortList += '<tr><td>Minimum Payout</td><td>' + value.paymentProcessing.minimumPayment + '</td></tr>';
                    //if (typeof(value.paymentProcessing.minimumPaymentToPaymentId) !== "undefined") {
                    //    PoolPortList += '<tr><td>Minimum Payout via Exchange</td><td>' + value.paymentProcessing.minimumPaymentToPaymentId + '</td></tr>';
                    //}
                    //PoolPortList += '<tr><td>Pool Fee</td><td>' + value.poolFeePercent + '%</td></tr>';
                    $.each(value.ports, function (port, options) {
                        PoolPortList += '<tr><td>Port ' + port + ' Difficulty</td><td>';
                        if (typeof(options.varDiff) !== "undefined") {
                            PoolPortList += 'Variable / ' + options.varDiff.minDiff + ' &harr; ';
                            if (typeof(options.varDiff.maxDiff) === "undefined") {
                                PoolPortList += '&infin;';
                            } else {
                                PoolPortList += options.varDiff.maxDiff;
                            }
                        } else {
                            PoolPortList += 'Static / ' + options.difficulty;
                        }
                        PoolPortList += '</td></tr>';
                    });
                }
            ...
            PoolPortList += '</tbody>';
            $('#poolPortList').html(PoolPortList);
        })
        .fail(function () {
            $.notify({
                icon: "ti-cloud-down",
